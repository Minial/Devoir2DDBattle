...
class Mob:
    ...
    def deplacement(self, idMobVise):
        ...
        elif (distance[0] <= self.speed + self.rangeMelee):
            # Déplacement en range melee (possibilité d'attaque)
        ...
        elif (distance[0] <= 2*self.speed + self.rangeMelee):
            # Déplacement en range melee (impossible d'attaquer)
            x = self.position[0] + (dep - self.rangeMelee)*Xtotal / l
            y = self.position[1] + (dep - self.rangeMelee)*Ytotal / l
            self.destination = [x,y]
